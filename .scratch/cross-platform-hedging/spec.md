...
- During initial registration, the worker operator enters the MT5 password through the same visible input behavior as the other enrollment fields. After successful local MT5 login, the native worker sends the password alongside actual account/terminal evidence and a hardware-key proof through Cloudflare TLS. The controller immediately stores the password in OpenBao, never logs it, and deletes the pending secret when registration expires or is rejected.
