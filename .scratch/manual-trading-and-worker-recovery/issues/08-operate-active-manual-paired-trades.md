...
- [x] A target has at most one active manual paired trade and exposes one
  preview-confirmed full-exit action that operates both legs.
- [x] A protection update previews the mapped broker TP/SL values for both
  actual fills, then updates both legs from one shared positive pip-distance
  request.
- [x] Full-exit and protection-update retries are idempotent; failed or
