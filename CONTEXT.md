...
**跨伺服器商品配對**:
一個無方向的商品關係，由兩個交易伺服器各自的一個商品組成，經人工審核後可建立相反等值曝險。每個伺服器端點以完整、大小寫精確的已驗證 MT5 `server` 字串識別；同品牌的不同 server 仍是不同端點。主方向與避險方向由共同交易意圖決定，不屬於商品配對本身。第一版外匯商品配對一律以 1:1 lot 關係表達；帳戶風險 sizing 由共同交易意圖處理。主控台只使用已通過審核並列入可下單清單的跨伺服器商品配對，以共同 pip 度量補救避險偏差。同一組無方向的伺服器／商品兩端在任一時刻至多有一個 active 跨伺服器商品配對；替代版本以原子 Replace 同時 retire 現行配對並建立新 active 配對。
_Avoid_: Mapping
