...
**商品配對分析**:
由主控台向兩個不同 MT5 server 上、已批准、健康且已連線的帳戶工作者發起的唯讀比較工作。它先比較商品目錄與規格，再以主控台指定的 UTC 完整交易週之 M15 資料篩選候選，並以 M1 資料驗證；worker 必須保存原始 broker epoch 與市場資料時間校正證據。第一版僅分析外匯商品：兩端的 base/profit currency 必須相同，且 broker 標示為 FOREX；不符 calc mode 的商品僅顯示為人工例外，不能自動分析或建立。分析以分析政策快照判定通過與否。每個階段皆須取得兩端完整且帶時間戳的回覆；逾時、斷線或資料不完整使分析失敗，不產生可建立的部分候選。每個工作者一次只可參與一個分析，其他工作排隊；分析失敗最多重試一次。分析只保存不可變摘要、規格、時間校正與資料雜湊，不保存完整 market bars，且至少保留一年；Web UI 不得刪除。
