...
**管理站 UI 時間格式**:
所有管理站 Web UI 的時間戳一律顯示為 UTC `YYYY-MM-DD HH:mm:ss`，不得附加 `T`、時區 suffix 或 locale-dependent 格式。手動交易報價僅顯示 broker quote time，不顯示 controller receipt time。
...
**管理員手動交易頁**:
管理站中獨立於管理員意圖工作區的操作面。管理員可隨時編輯目前的手動交易 target；target 指定來自配對兩端的 Buy 與 Sell worker，進場一律沿用該 target。它為 target 顯示雙端即時報價，並把 scheduled、active 與 needs_human 手動交易呈現為精簡的配對紀錄；每筆紀錄保存每一 leg 的 worker、broker 市價單 ticket、position ticket 與目前 position 狀態，且各 active trade 各有獨立的 Close 操作。
