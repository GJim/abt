...
**交易者註冊**:
尚未授權的交易者以新生成、不可匯出的 P-256 公鑰及其簽署證明，經 API 聲明策略名稱與公開 IP 的程序；註冊需要管理員預先簽發、一次且短期有效的 invite，主控台分配 registration ID，管理員依該聲明 IP、策略名稱與金鑰指紋審核後決定是否簽發交易者裝置憑證。待審核註冊在 15 分鐘後過期，拒絕與過期的 registration ID 均不得重用；交易者必須重新註冊。公開 IP 是交易者的宣告資料，不保存或比對 Cloudflare 觀測來源。交易者註冊不使用 MT5 證據或人工配對碼。
...
**交易者裝置憑證**:
主控台簽發的交易者部署身分證明，綁定單一 trader ID、不可匯出的 ECDSA P-256 公鑰與有效期限；私鑰只能保存在 CNG、TPM 2.0 或 PKCS#11 keystore。憑證有效期為 30 天，交易者以有效舊私鑰簽署輪替請求並生成新 key pair；主控台只在舊憑證未撤銷時簽發新憑證。
...
**管理員意圖工作區**:
管理站中供管理員建立與處理共同交易意圖的操作面；預設顯示活動意圖，並提供可篩選的完整歷史與每筆意圖不可變事件軌跡。建立與一般取消均先顯示 pair、方向、lots、entry、SL/TP、FOK/IOC 與到期的預覽，再要求明確確認。只有兩腿均零成交的意圖可顯示一般取消；已成交或凍結意圖只能進入緊急平倉處置。
...
**管理員緊急平倉**:
主控台管理員對任一已成交配對執行的受稽核風險處置；主控台取消未成交腿並平掉已成交腿，失敗時凍結配對。它不屬於交易者撤單，交易者不得觸發，且不要求輸入理由；管理站只要求顯示預覽後按明確確認按鈕。
...
**IOC 部分成交淨額處置**:
IOC 進場後，主控台以兩腿實際成交量依商品配對比例換算可匹配量，保留該較小量為已避險配對，先取消兩腿剩餘未成交訂單並以即時對帳確認，再以市價平掉較大腿的未避險差額。不進行 IOC 補單；若沒有可匹配量，平掉所有已成交量。任何取消、平倉或即時對帳無法確認時，凍結配對並要求人工復歸。
...
**不確定恢復結果**:
取消或市價平倉因斷線而未取得 broker 結果的狀態；配對必須凍結並先完成即時對帳，才可再次發出寫入命令。
_Avoid_: 立即重試
