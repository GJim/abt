...
from .wine_mt5_protocol import MAX_FRAME_BYTES, PROTOCOL_VERSION, ProtocolError, decode_json_object, encode_frame
...
class BridgeRemoteError(BridgeClientError):
    """The bridge returned a structured failure before reporting success."""


class MutationOutcomeUnknown(BridgeClientError):
    """A one-shot mutation request was sent but no trustworthy receipt arrived."""
...
class BridgeClient:
    def __init__(
        ...
        *,
        ...
        self.one_shot_mutation = one_shot_mutation
        command = ["wine", windows_python, _wine_path(HERE / "wine_mt5_bridge.py")]
        if one_shot_mutation:
            command.append("--one-shot-mutation")
    ...
    def request(self, operation: str, params: dict[str, Any] | None = None) -> Any:
        ...
        if response.get("ok") is not True:
            ...
            raise BridgeRemoteError(str(message))
        return response.get("result")

    def close(self) -> None:
        if self.process.poll() is None and not self.one_shot_mutation:
            try:
...
class OneShotMutationClient:
    """Execute exactly one MT5 mutation in a dedicated Wine child without retrying."""
        ...
        try:
            try:
                ...
                raise MutationOutcomeUnknown(
                    "MT5 mutation outcome is unknown; do not retry automatically and reconcile broker state."
                ) from error
        finally:
            client.close()
