...
readonly project_directory="$(cd "${deploy_directory}/.." && pwd)"
...
elif uv run --project "${project_directory}" python -c 'import json' >/dev/null 2>&1; then
  python_command=(uv run --project "${project_directory}" python)
else
  printf 'A working Python interpreter is required. Install Python 3 or install uv in PATH.\n' >&2
  exit 1
fi
...
for _ in {1..30}; do
  ...
  if "${python_command[@]}" -c 'import json, sys; json.load(sys.stdin)' <<<"${candidate}" >/dev/null 2>&1; then
    status="${candidate}"
    break
  fi
  sleep 2
...
initialized="$("${python_command[@]}" -c 'import json, sys; print(json.load(sys.stdin)["initialized"])' <<<"${status}")"
...
root_token="$("${python_command[@]}" -c 'import json, sys; print(json.load(sys.stdin)["root_token"])' <"${init_file}")"
...
health_token="$(bao token create -format=json -display-name=abt-health -policy=abt-health |
  "${python_command[@]}" -c 'import json, sys; print(json.load(sys.stdin)["auth"]["client_token"])')"
controller_token="$(bao token create -format=json -display-name=abt-controller -policy=abt-controller |
  "${python_command[@]}" -c 'import json, sys; print(json.load(sys.stdin)["auth"]["client_token"])')"

"${python_command[@]}" - "${env_file}" "${health_token}" "${controller_token}" <<'PYTHON'
import os
import sys
