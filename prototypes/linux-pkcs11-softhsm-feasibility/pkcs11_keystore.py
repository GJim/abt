...
class IdentityExistsError(IdentityStateError):
    pass
...
def _require_mode(path: Path, expected: int) -> None:
    try:
        ...
        raise IdentityStateError(f"identity state is missing: {path.name}") from error
    if actual != expected:
        raise IdentityStateError(f"identity state has unsafe permissions: {path.name}")
...
def _check_rc(result: int, operation: str, *, allowed: tuple[int, ...] = ()) -> None:
    if result != _CK_OK and result not in allowed:
        raise IdentityStateError(f"{operation} failed (0x{result:08X})")
...
def _initialize_token_locked(module_path: Path, config_path: Path, token_label: str, so_pin: bytes, user_pin: bytes) -> None:
    ...
    try:
        ...
        if _OPEN_MODULE_COUNTS.get(module_key, 0):
            raise IdentityStateError("all PKCS#11 key stores must be closed before provisioning")
        ...
        if count.value == 0:
            raise IdentityStateError("SoftHSM exposes no slots")
        ...
        if len(label) > 32:
            raise IdentityStateError("token label exceeds 32 bytes")
        ...
        if not initialized:
            raise IdentityStateError("could not initialize the SoftHSM user PIN")
...
def _activate_library(module_path: Path, config_path: Path) -> Any:
    ...
    try:
        ...
        if library is None:
            ...
            if _OPEN_MODULE_COUNTS.get(module, 0):
                raise IdentityStateError("another PKCS#11 identity is active; close it before switching")
...
class PKCS11KeyStore:
    def __init__(self, identity_path: Path, module_path: Path, metadata: dict[str, str]) -> None:
    ...
    @classmethod
    def open(cls, identity_path: Path | str, *, module_path: Path | str) -> "PKCS11KeyStore":
        with _LOCK:
            return cls._open_locked(identity_path, module_path=module_path)

    @classmethod
    def _open_locked(cls, identity_path: Path | str, *, module_path: Path | str) -> "PKCS11KeyStore":
        ...
        if not module.is_file():
            raise IdentityStateError("PKCS#11 module is unavailable")
        try:
            metadata = json.loads((path / "identity.json").read_text(encoding="utf-8"))
            pin = (path / "user-pin").read_text(encoding="ascii")
        except (OSError, UnicodeError, json.JSONDecodeError) as error:
            raise IdentityStateError("identity state is unreadable") from error
        if set(metadata) != {"provider", "token_label", "key_id", "key_label"} or metadata["provider"] != "PKCS11-SOFTWARE":
            raise IdentityStateError("identity metadata is invalid")
        store = cls(path, module, metadata)
        try:
            store._open_session(pin)
        except Exception as error:
            store.close()
            if isinstance(error, IdentityStateError):
                raise
            raise IdentityStateError("SoftHSM login or key lookup failed") from error
        return store
    ...
    def _require_open(self) -> None:
        if self._session is None or self._private_key is None or self._public_key is None:
            raise IdentityStateError("PKCS#11 key store is closed")
    ...
    def close(self) -> None:
        with _LOCK:
            ...
            if close_error is not None:
                raise IdentityStateError("PKCS#11 session close failed") from close_error
...
def _rename_noreplace(source: Path, destination: Path) -> None:
    ...
    if renameat2 is None:
        raise IdentityStateError("atomic no-replace rename is unavailable on this Linux host")
    ...
    raise IdentityStateError(f"atomic identity publish failed: {os.strerror(error_number)}")


def provision_identity(identity_path: Path | str, *, module_path: Path | str) -> PKCS11KeyStore:
    with _LOCK:
        return _provision_identity_locked(identity_path, module_path=module_path)


def _provision_identity_locked(identity_path: Path | str, *, module_path: Path | str) -> PKCS11KeyStore:
                ...
                try:
                    with token.open(rw=True, user_pin=user_pin.decode("ascii")) as session:
                        ...
                        if not isinstance(public_key, ec.EllipticCurvePublicKey):
                            raise IdentityStateError("generated key is not an EC public key")
                        public_key.verify(
    ...
    try:
        store = PKCS11KeyStore.open(final_path, module_path=module)
    except Exception:
...
def delete_identity(identity_path: Path | str) -> None:
    ...
    with _LOCK:
        if not path.exists():
            raise IdentityStateError("identity does not exist")
        _require_mode(path, 0o700)
        if _OPEN_PATH_COUNTS.get(str(path.resolve()), 0):
            raise IdentityStateError("identity is in use; close all key stores before deleting it")
        shutil.rmtree(path)
