...
def parse_log(path: Path) -> dict:
    ...
    for line in lines:
        ...
        if match:
            closes.append({"attempt": match.group(1), "reason": match.group(2)})
        if "profit_trail_applied state=PROTECTING" in line:
            trails += 1
        if "asymmetric_protection_applied state=PROTECTING" in line:
            asym += 1
