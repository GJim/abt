...
export function IntentWorkspacePage({ csrfToken }: { csrfToken: string }) {
  ...
  return <section aria-label="Intent workspace">
    <div className="console-page-header intent-controls"><label><input checked={history} onChange={(event) => setHistory(event.target.checked)} type="checkbox" /> Show complete history</label><label>Status <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All statuses</option><option value="accepted">Accepted</option><option value="working">Working</option><option value="needs_human">Needs review</option><option value="cancelled">Cancelled</option></select></label></div>
